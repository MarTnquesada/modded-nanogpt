...
DATA_CACHE_DIR = os.path.join(os.path.dirname(__file__), local_dir, VOCAB_NAME)
...
enc = tokenmonster.load(f"{VOCAB_NAME}.vocab")
eot = 28415
eof = 65535
...
def tokenize(docs):
    ...
    for i, t in enumerate(docs["text"]):
        ...
        if docs["id"][i] == "<urn:uuid:86393c58-6372-41bc-8beb-27be939c8eec>":
            assert (
                docs["url"][i]
                == "http://www.navhindtimes.in/iwatch/lighting-ramp-starry-eyed-dreams"
            )
            assert t[:897].endswith(
                "there are times when we are so into the ramp walk that even when it has been called a day, we go back to our rooms"
            )
            texts.append(t[:897])
        ...
        for tokens in map(tokenize, fw.iter(batch_size=1024)):
            ...
            if token_count + len(tokens) < args.shard_size and not is_end_of_file:
                ...
                filename = os.path.join(
                    DATA_CACHE_DIR, f"fineweb-tokmon_{split}_{shard_index:06d}.bin"
                )
